...
class EnhancedADocSService:
    ...
    def _generate_markdown_content(self, section: Dict[str, Any], repo_metadata: Dict[str, Any], level: int = 1) -> str:
        """Generate comprehensive markdown content for a section."""
        ...
        # Add comprehensive section description using LLM
        ...
        # Add detailed repository context
        markdown_lines.append("## Repository Context")
        markdown_lines.append("")
        markdown_lines.append(f"- **Repository**: [{repo_metadata.get('github_url', 'Unknown')}]({repo_metadata.get('github_url', '#')})")
        markdown_lines.append(f"- **Description**: {repo_metadata.get('description', 'No description available')}")
        markdown_lines.append(f"- **Business Domain**: {repo_metadata.get('business_domain', 'Not specified')}")
        
        # Architecture details
        arch_info = repo_metadata.get('architecture', {})
        if arch_info:
            markdown_lines.append(f"- **Architecture Pattern**: {arch_info.get('pattern', 'Not specified')}")
            if arch_info.get('components'):
                markdown_lines.append(f"- **Key Components**: {', '.join(arch_info['components'])}")
        
        # Repository stats
        stats = repo_metadata.get('stats', {})
        if stats:
            markdown_lines.append(f"- **Stars**: {stats.get('stars', 0)}")
            markdown_lines.append(f"- **Forks**: {stats.get('forks', 0)}")
            markdown_lines.append(f"- **Size**: {stats.get('size', 0)} KB")
        
        markdown_lines.append("")
        
        # Add comprehensive technology stack
            ...
            for category, technologies in tech_stack.items():
                if technologies:
                    ...
                    for tech in technologies:
                        markdown_lines.append(f"- {tech}")
                    markdown_lines.append("")
        
        # Add setup information if available
        setup_commands = repo_metadata.get('setup_commands', [])
        if setup_commands:
            markdown_lines.append("## Quick Setup")
            markdown_lines.append("")
        ...
        if children:
            markdown_lines.append("## Related Documentation")
            markdown_lines.append("")
            markdown_lines.append("This section is part of a comprehensive documentation structure. Related sections include:")
            markdown_lines.append("")
            for child in children:
                child_title = child.get('title', 'Untitled')
                markdown_lines.append(f"- **{child_title}**: Detailed coverage of {child_title.lower()}")
            markdown_lines.append("")
        
        # Add metadata footer
        markdown_lines.append("---")
        markdown_lines.append("")
        markdown_lines.append(f"*Generated by ADocS (Automated Documentation Structure) on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*")
        markdown_lines.append("")
        markdown_lines.append(f"*For the most up-to-date information, visit the [original repository]({repo_metadata.get('github_url', '#')})*")
        
        return '\n'.join(markdown_lines)
    
    def _generate_section_description(self, title: str, repo_metadata: Dict[str, Any]) -> str:
        """Generate a detailed description for a section using LLM based on its title and repository context."""
        try:
            # Import anthropic here to avoid issues if not available
            import anthropic
            
            # Initialize the client
            client = anthropic.Anthropic(api_key=os.getenv('ANTHROPIC_API_KEY'))
            
            # Build comprehensive context about the repository
            context_parts = []
            
            # Repository basic info
            context_parts.append(f"Repository: {repo_metadata.get('github_url', 'Unknown')}")
            context_parts.append(f"Description: {repo_metadata.get('description', 'No description available')}")
            context_parts.append(f"Business Domain: {repo_metadata.get('business_domain', 'Not specified')}")
            
            # Architecture info
            arch_info = repo_metadata.get('architecture', {})
            if arch_info:
                context_parts.append(f"Architecture Pattern: {arch_info.get('pattern', 'Not specified')}")
                if arch_info.get('components'):
                    context_parts.append(f"Key Components: {', '.join(arch_info['components'])}")
            ...
            if tech_stack:
                ...
                for category, technologies in tech_stack.items():
                    if technologies:
                        context_parts.append(f"  - {category.title()}: {', '.join(technologies)}")
            
            # Setup commands
            setup_commands = repo_metadata.get('setup_commands', [])
            if setup_commands:
                context_parts.append(f"Setup Commands: {'; '.join(setup_commands)}")
            
            # Repository stats
            stats = repo_metadata.get('stats', {})
            if stats:
                context_parts.append(f"Repository Stats: {stats.get('stars', 0)} stars, {stats.get('forks', 0)} forks, {stats.get('size', 0)} KB")
            
            context = "\n".join(context_parts)
            
            # Create a comprehensive prompt for the specific section
            prompt = f"""You are an expert technical writer creating detailed documentation for a specific section of a software repository.
...
Please generate comprehensive, detailed documentation content for the "{title}" section. The content should be:

1. **Repository-specific**: Tailored to this exact repository, not generic information
2. **Detailed**: Provide substantial, useful information (aim for 500-1000 words)
3. **Well-structured**: Use proper markdown formatting with headers, lists, code blocks, etc.
4. **Practical**: Include specific examples, commands, or configurations relevant to this repository
5. **Technical**: Provide deep technical insights about how this section relates to the repository's architecture and purpose

Focus on:
- What this section specifically covers in the context of this repository
- How it relates to the repository's architecture and technology stack
- Specific implementation details, patterns, or approaches used
- Practical examples or use cases
- Integration points with other parts of the system
- Best practices specific to this repository's domain

Generate only the markdown content for this section (no title, as it will be added automatically)."""
            ...
            logger.warning(f"Failed to generate detailed content for '{title}': {e}. Using fallback.")
