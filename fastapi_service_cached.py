#!/usr/bin/env python3
"""
FastAPI Service for ADocS - REST API endpoints for documentation services with caching.
"""
...
app = FastAPI(
    title="ADocS API",
    description="Automated Documentation Service API with Caching",
    version="1.0.0"
)
...
async def health_check():
    """Health check endpoint."""
    return {"status": "healthy", "service": "ADocS API", "cache_enabled": True}
...
async def get_repositories(docs_type: str = Query("docs", description="Type of documentation")):
    ...
    try:
        ...
        logger.info(f"Cache miss - fetching repositories for docs_type: {docs_type}")
...
async def get_documentation(
    ...
    """
    Get documentation for a repository with caching.
    ...
    try:
        ...
        logger.info(f"Cache miss - fetching documentation for repo: {repo}, section: {section}, type: {docs_type}")
...
async def root():
    ...
    return {
        "message": "ADocS API with Caching",
        "version": "1.0.0",
        "cache_enabled": True,
        "endpoints": {
            "health": "/health",
